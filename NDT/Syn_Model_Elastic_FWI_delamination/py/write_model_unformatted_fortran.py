...
def write_fortran(filename,dat):
    """ Reads Fortran style binary data and returns a numpy array.
    """
    with open(filename, 'wb') as f:
        dat.ravel(order='F').tofile(f)
    return
...
def mesh2grid(v, x, z):
    ...
    nx = 800 # Heidimod model gridsize is 800x400
    nz = 400
    dx = lx/nx
    dz = lz/nz
    ...
    V = scipy.interpolate.griddata(mesh, v, grid, 'nearest')
...
if __name__ == '__main__':
    """ Plots data on 2-D unstructured mesh
      ...
      SYNTAX
          plot2d  x_coords_file  z_coords_file  database_file database_out_file
    """
    ...
    # write fortran
    write_fortran(database_out_file,V)
