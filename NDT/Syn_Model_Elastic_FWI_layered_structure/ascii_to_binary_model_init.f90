...
program sum_kernels
  ...
  integer :: ipath, npath, iker, nline, i,j, dummy
...
subroutine get_number_lines(filename, nline)
  ...
  real(kind=CUSTOM_REAL) :: dummy0, dummy1, dummy2, dummy3, dummy4, dummy5
  ...
  do j=1,MAX_NUMBER_LINES
     read(100,*,iostat=ios) dummy0, dummy1, dummy2, dummy3, dummy4, dummy5
     !read(100,*,iostat=ios) dummy1, dummy2
     if (ios /= 0) exit
     nline=nline+1
  enddo
  close(100)
