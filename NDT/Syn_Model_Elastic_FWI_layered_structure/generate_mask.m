function generate_mask
    ...
    W(Z > zmax - 25) = 0.;
    span = 4; %2
    W = gridsmooth(W,span,'gauss'); %'moving'
    %W = scale(W,[0 1]);

    %%% load gll coordinates
    [l,x,z] = loadbycol('DATA/proc000000_rho_vp_vs.dat_output');
    n = length(x);

    %%% interpolate to gll coordinates
    w = griddata(X,Z,W,x,z);
    mask = [l x z zeros(n,1) w w];
    saveascii(mask, 'DATA/mask_ascii')
